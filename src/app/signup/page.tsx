    ...
    <div className="container mx-auto flex min-h-[80vh] items-center justify-center px-4 py-12">
      <div className="w-full max-w-md rounded-2xl border bg-card p-8 shadow-xl space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-muted-foreground">Join the AI Pulse blogging ecosystem</p>
        </div>
            ...
            <div className="grid grid-cols-3 gap-2 mt-1">
              {(['READER', 'AUTHOR', 'ADMIN'] as const).map((r) => (
                <button
                  ...
                  key={r}
                  onClick={() => setRole(r)}
                  className={`rounded-xl border py-2 text-xs font-semibold ${role === r ? 'bg-purple-600 text-white border-purple-600' : 'bg-background hover:bg-muted'}`}
                >
                  {r}
                </button>
              ))}
